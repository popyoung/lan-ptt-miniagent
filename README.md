...
- 启动后默认停留在系统托盘图标;托盘和主窗口使用项目内应用图标;单击/双击托盘图标或右键"打开主界面"可显示窗口,关闭窗口会隐藏回托盘。
...
9. 程序默认在系统托盘运行;关闭主窗口只会隐藏到托盘。右键托盘图标可"打开主界面"或"退出",只有点击"退出"才会真正停止监听并结束进程。
...
"启用麦克风语音增强"只处理本机麦克风发送前的 PCM16 单声道帧,不改变 UDP 协议。处理链使用 NWaves 在线高通滤波器和动态限幅器,再配合保守 RMS 自动增益、清晰度频段补偿和强度相关的发送端 makeup gain,用于减少低频/DC 偏移、抬高较小的人声,并让高强度对正常音量人声也有可感知的响度差异。强度越高,增益、高通截止和清晰度补偿越明显;默认关闭,打开后默认强度 50。接收端仍会按界面上的"音量"设置缩放播放帧。

`maxGainMultiplier` 是语音增强的最大自动增益倍数,范围 `4..100`,默认 `8`。非常高的增益只适合安静环境中的弱麦克风,会明显放大背景噪声、喷麦和环境声。
